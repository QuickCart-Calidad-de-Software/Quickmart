...
export default function ReportView() {
  ...
  const usersDataByPeriod: Record<Period, UsersData[]> = {
    ...
    week: [
      ...
      { period: "Semana 44", newUsers: 103, activeUsers: 1591, growth: "+6.9%" }
    ],
    month: [
  ...
  const totalNewUsers = currentUsersData.reduce((sum, item) => sum + item.newUsers, 0);
      ...
      {reportGenerated && (
        <>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                >
                  ...
                  <p className="text-2xl font-bold text-purple-600 dark:text-purple-400">
                    ${Math.round(totalSales * 150 / totalOrders).toLocaleString()}
                  </p>
                  <p className="text-xs text-green-600 dark:text-green-400 mt-1">
                    ↑ +12.5% vs anterior
                  </p>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                >
                  ...
                  <p className="text-2xl font-bold" style={{ color: "var(--foreground)" }}>
                    {currentUsersData[currentUsersData.length - 1].activeUsers.toLocaleString()}
                  </p>
                  <p className="text-xs text-green-600 dark:text-green-400 mt-1">
                    ↑ Usuarios activos
                  </p>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            >
                  ...
                  >
                    ...
                    <Tooltip 
                      ...
                      formatter={(value: any, name: string) => {
                        if (name === 'ventas') {
                          const dataPoint = currentSalesData.find(d => d.sales === value);
                          return [
                            <div key="tooltip" className="space-y-1">
                              <div className="font-bold text-blue-600 dark:text-blue-400">{value} ventas</div>
                              <div className="text-sm text-gray-600 dark:text-gray-400">{dataPoint?.revenue}</div>
                              <div className="text-xs text-gray-500 dark:text-gray-500">{dataPoint?.orders} pedidos</div>
                            </div>,
                            ''
                          ];
                        }
              ...
              <div className="grid grid-cols-3 gap-4 mt-6 pt-4 border-t" style={{ borderColor: "var(--border-color)" }}>
                ...
                <div className="text-center">
                  <p className="text-xs text-gray-500 dark:text-gray-400">Mayor valor</p>
              ...
              <div className="relative px-2" style={{ height: "280px" }}>
                <div className="absolute inset-0 flex items-end justify-between pb-10 px-2">
                  {currentUsersData.map((item, index) => {
                    const maxValue = Math.max(...currentUsersData.map(d => d.activeUsers));
                    const minValue = Math.min(...currentUsersData.map(d => d.activeUsers));
                    ...
                    const heightPercent = range > 0 ? 35 + (normalizedValue / range) * 65 : 100;
                    
                    return (
                      <div key={index} className="flex flex-col items-center justify-end group" style={{ width: `${85 / currentUsersData.length}%` }}>
                        {/* Value on hover */}
                        ...
                        >
                          <span className="text-white font-bold text-sm drop-shadow">
                            {item.activeUsers >= 1000 ? `${(item.activeUsers / 1000).toFixed(1)}k` : item.activeUsers}
                          </span>
                        </div>
                      </div>
                    );
                ...
                {/* X-axis labels */}
                <div className="absolute bottom-0 left-0 right-0 flex justify-between px-2 border-t pt-2" style={{ borderColor: "var(--border-color)" }}>
                  {currentUsersData.map((item, index) => (
                    <div key={index} className="text-center" style={{ width: `${85 / currentUsersData.length}%` }}>
              ...
              <div className="grid grid-cols-3 gap-4 mt-6 pt-4 border-t" style={{ borderColor: "var(--border-color)" }}>
                <div className="text-center">
                  ...
                  <p className="text-lg font-bold text-orange-600 dark:text-orange-400">
                    {Math.min(...currentUsersData.map(d => d.activeUsers)).toLocaleString()}
                  </p>
                </div>
                <div className="text-center">
                  <p className="text-xs text-gray-500 dark:text-gray-400">Promedio</p>
                  <p className="text-lg font-bold text-blue-600 dark:text-blue-400">
                    {Math.round(currentUsersData.reduce((sum, d) => sum + d.activeUsers, 0) / currentUsersData.length).toLocaleString()}
                  </p>
                </div>
                <div className="text-center">
                  <p className="text-xs text-gray-500 dark:text-gray-400">Mayor valor</p>
                  <p className="text-lg font-bold text-green-600 dark:text-green-400">
                    {Math.max(...currentUsersData.map(d => d.activeUsers)).toLocaleString()}
                  </p>
                </div>
              </div>
            </div>
          ...
          {outputFormat === "table" && (
            ...
            >
              <h3 className="text-lg font-semibold mb-4" style={{ color: "var(--foreground)" }}>
                Datos Detallados - {reportType === "sales" ? "Ventas" : "Usuarios"}
              </h3>
      ...
      {!reportGenerated && (
        ...
        >
          ...
          <p className="text-gray-500 dark:text-gray-400">
            Datos de ejemplo mostrados - Ajusta los filtros para regenerar el reporte
          </p>
        </div>
      )}
    </div>
