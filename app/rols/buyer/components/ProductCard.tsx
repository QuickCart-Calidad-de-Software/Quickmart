...
export default function ProductCard({ product, onAddToCart }: ProductCardProps) {
  ...
  return (
    <div className="group bg-white border border-neutral-200 overflow-hidden hover-lift hover:shadow-2xl transition-all duration-500 rounded-sm">
      {/* Image - Parte superior */}
      <div className="relative aspect-square overflow-hidden bg-neutral-100">
        ...
        {hasDiscount && (
          <div className="absolute top-4 left-4 bg-white text-neutral-900 px-4 py-2 rounded-sm">
        ...
        {urgencyMessage && (
          <div className="absolute top-4 right-4 border border-white bg-neutral-900/90 backdrop-blur-sm text-white text-xs font-normal px-3 py-1.5 rounded-sm uppercase tracking-wider">
            {urgencyMessage}
          </div>
        )}
        ...
        {(product.id === '2' || product.id === '7') && (
          <div className="absolute bottom-4 left-4 border border-white bg-neutral-900/90 backdrop-blur-sm text-white text-xs font-normal px-3 py-1.5 rounded-sm uppercase tracking-widest">
            NEW ARRIVAL
          </div>
        )}
      </div>

      {/* Content - Parte inferior BLANCA como Maguire */}
      <div className="p-8 space-y-6 bg-white">
        {/* Category Badge */}
        <div className="inline-block bg-neutral-100 text-neutral-600 px-3 py-1 rounded-sm uppercase tracking-widest text-xs">
          {product.category}
        </div>

        {/* Title */}
        <h3 className="font-light text-neutral-900 text-xl mb-3 line-clamp-2 min-h-[3.5rem] tracking-tight">
          {product.name}
        </h3>

        {/* Rating */}
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2 bg-neutral-100 px-3 py-1.5 rounded-sm">
        ...
        <div className="space-y-3 text-sm">
          ...
          <div className="text-neutral-500 truncate font-light">
            By <span className="text-neutral-700 font-normal">{product.sellerName}</span>
          </div>
        </div>
        ...
        <div className="pt-6 border-t border-neutral-200 space-y-3">
            ...
            <div>
              ...
              {hasDiscount && (
                <div className="flex items-center gap-3 mt-1">
                  ...
                  <span className="text-xs text-neutral-900 uppercase tracking-widest font-normal">
          ...
          {product.price >= 5000 && (
            <div className="text-xs text-neutral-600 uppercase tracking-widest font-light">
              Free Shipping Included
            </div>
          )}
        </div>

        {/* Add to Cart Button - NEGRO sobre blanco */}
        <button
        ...
        {urgencyMessage && product.stock <= 5 && (
          <p className="text-center text-xs text-neutral-500 font-light italic">
